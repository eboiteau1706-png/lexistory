...
export default function ClassementPage() {
  ...
  const [mondayDate, setMondayDate]     = useState<string | null>(null);
  ...
  useEffect(() => {
    fetch("/api/leaderboard")
      .then(r => r.json())
      .then(({ players }) => { setPlayers((players as Player[]) ?? []); setLoading(false); })
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      supabase.from("profiles").select("id, username, xp, is_premium, avatar_url").eq("id", uid).single()
        .then(({ data }) => {
          if (!data?.username) return;
          setPlayers(prev => prev.some(p => p.id === uid) ? prev : [...prev, data as Player].sort((a, b) => b.xp - a.xp));
        });
    });
  }, []);

  useEffect(() => {
    if (tab !== "semaine" || weeklyLoaded) return;
    setWeeklyLoading(true);
    fetch("/api/classement-semaine")
      .then(r => r.json())
      .then(({ players, mondayDate }) => {
        setWeeklyPlayers((players as Player[]) ?? []);
        setMondayDate(mondayDate ?? null);
  ...
  async function loadFriendStatus(uid: string) {
    const { data } = await supabase.from("friendships").select("user_id, friend_id, status").or(`user_id.eq.${uid},friend_id.eq.${uid}`);
    if (!data) return;
    const accepted: string[] = [], pending: string[] = [];
    data.forEach((f: any) => {
      const o = f.user_id === uid ? f.friend_id : f.user_id;
      if (f.status === "accepted") accepted.push(o); else pending.push(o);
    });
    setFriendIds(accepted); setPendingIds(pending);
  }

  async function loadFriends(uid: string) {
    const { data: allF } = await supabase.from("friendships").select("user_id, friend_id").or(`user_id.eq.${uid},friend_id.eq.${uid}`).eq("status", "accepted");
    ...
    const ids = [...allF.map((f: any) => f.user_id === uid ? f.friend_id : f.user_id), uid];
    const { data: profiles } = await supabase.from("profiles").select("id, username, xp, is_premium, avatar_url").in("id", ids).not("username", "is", null).order("xp", { ascending: false });
  ...
  async function sendFriendRequest(targetId: string, targetUsername: string) {
    ...
    if (error) setAddMsg(prev => ({ ...prev, [targetId]: "❌ Erreur" }));
    else { setPendingIds(prev => [...prev, targetId]); setAddMsg(prev => ({ ...prev, [targetId]: "✅ Demande envoyée !" })); }
    setAddingId(null);
    setTimeout(() => setAddMsg(prev => { const n = { ...prev }; delete n[targetId]; return n; }), 3000);
  }

  function renderRow(player: Player, idx: number, displayXp: number, keyPrefix = "") {
    ...
    return (
      <div key={`${keyPrefix}${player.id}`} className={`${styles.row} ${isMe ? styles.rowMe : ""} ${idx < 3 ? styles.rowTop : ""}`}>
        <div className={styles.rank}>{medal || <span className={styles.rankNum}>#{idx + 1}</span>}</div>
        <div style={{ width: 40, height: 40, minWidth: 40, borderRadius: "50%", flexShrink: 0, overflow: "hidden", background: avatarBg(player.username ?? "?"), display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.9rem", fontWeight: 700, color: "#fff", alignSelf: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0, minWidth: "120px", justifyContent: "flex-end" }}>
          <div className={styles.xp}>{displayXp} XP</div>
          {isMe ? (
  ...
  const list = tab === "global" ? players : tab === "amis" ? friends : weeklyPlayers;
  const getDisplayXp = (p: Player) => tab === "semaine" ? (p.weeklyXp ?? 0) : p.xp;
  const myRankIndex  = list.findIndex(p => p.id === myId);
  const myRank       = myRankIndex + 1;
  const isInTop10    = myRankIndex >= 0 && myRankIndex < 10;

  const isLoading = loading || (tab === "semaine" && weeklyLoading);
    ...
    <div className={styles.page}>
      <div className={styles.card}>
        ...
        {tab === "semaine" && mondayDate && (
          <div style={{ fontSize: "0.72rem", color: "var(--text-dim)", textAlign: "center", marginBottom: "4px" }}>
            Semaine du {new Date(mondayDate + "T12:00:00").toLocaleDateString("fr-FR", { day: "numeric", month: "long" })} · se renouvelle chaque lundi à 00h00
          </div>
        )}
          ...
          <>
            {myId && myRank > 0 && (
              <div className={styles.myRank}>
                ...
                {tab === "semaine" && <span style={{ color: "var(--text-dim)", fontSize: "0.82rem" }}> · {getDisplayXp(list[myRankIndex])} XP cette semaine</span>}
              </div>
            )}

            <div className={styles.list}>
              {list.slice(0, 10).map((player, i) => renderRow(player, i, getDisplayXp(player)))}
              ...
              {myId && !isInTop10 && myRankIndex >= 0 && (
                <>
                  ...
                  {[myRankIndex - 1, myRankIndex, myRankIndex + 1]
                    ...
                    .map(idx => renderRow(list[idx], idx, getDisplayXp(list[idx]), "nbr-"))}
                </>
              )}

              {list.length === 0 && (
                <div className={styles.empty}>
                  {tab === "amis" ? "Ajoute des amis pour les voir ici !"
                    : tab === "semaine" ? "Aucune activité cette semaine encore."
                    : "Aucun joueur trouvé."}
                </div>
              )}
            </div>
